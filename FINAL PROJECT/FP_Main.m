...
K = 6; % number of tree rows running south-north direction
M = 8; %maximum number of trees in each row
...
for j = 1:numTimesteps
    ...
    if j>=numTimesteps/10 && endDistance<1
        j
        endDistance
        E
...
plot(x_path,y_path,DisplayName="Desired Path")
...
plot(QEstMat(:,1),QEstMat(:,2),DisplayName="Estimated Locations",Color='y')
% viscircles(centersTree*gridResolution,radiiTree*gridResolution,'Color','g');
viscircles([reshape(MeasureX,[],1) reshape(MeasureY,[],1)],reshape(MeasureRadius,[],1),'Color','y','LineStyle','--');
...
ErrorX = x-MeasureX;
ErrorY = y-MeasureY;
ErrorRadius = 2*TreeRadius-2*MeasureRadius;
